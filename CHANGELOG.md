...
- **EUDISTACK-412 — Wallet tenant discovery plane (browser mode).** New public, session-less discovery endpoint `GET /business-wallet/.well-known/wallet-tenant-config` that resolves the requesting tenant from the `Host` header and returns its `wallet_mode` (plus `natural_persons_only`, `supported_credentials`, `version`) — cacheable on CloudFront with a 60-second TTL and an `ETag`; `key_manager` is deliberately never exposed in this public payload (AD-1bis). EUDIW browser wallets can read their tenant mode from this plane instead of the build-time `WALLET_MODE` env var (build-time fallback on failure). New `wallet.config` bounded context (hexagonal): domain model (`TenantWalletConfigDescriptor`, `WalletMode`, `KeyManager`, `ConfigurationAuditEvent`, `TenantWalletConfigInvariants` enforcing the wallet_mode/key_manager pairing rules — FR-20: browser ⇒ `key_manager` null); application services (`WalletTenantConfigReadService`, `TenantWalletConfigurationWriter`); R2DBC adapters that read/write `public.tenant_wallet_config` exclusively through a `search_path=public` connection so the discovery path never touches a tenant schema (AD-1, enforced by ArchUnit). Authenticated admin write API `POST /business-wallet/admin/wallet-tenant-config` (201) and `PUT /business-wallet/admin/wallet-tenant-config/{schemaName}` (200) with optimistic-lock `version` (409 on mismatch). Every config change is recorded in an append-only per-tenant audit table `<tenant>_business_wallet.wallet_config_audit` — actor, timestamp, before/after field deltas, outcome (`PERMIT`/`DENY`), correlation id — protected by a `BEFORE UPDATE OR DELETE` trigger. CloudFront cache invalidation is fired on each write (fire-and-forget: 3× exponential-backoff retry, failure recorded as a `CACHE_INVALIDATION_FAILED` audit row, never rolls back the write). A runbook (`docs/EUDISTACK-411-wallet-tenant-configuration/runbooks/register-browser-tenant.md`) documents the manual tenant-registration procedure. Implements EUDISTACK-411 US-01. (`eudistack-platform-iac` contributes the CloudFront cache policy + ordered behavior for the well-known path and the EBW task-role `cloudfront:CreateInvalidation` IAM policy.)
- **EUDISTACK-412 — Flyway migrations for the wallet discovery plane.** Added `db/migration/V2__Create_tenant_wallet_config.sql` creating `public.tenant_wallet_config` (column set aligned with `WalletTenantConfigEntity` and the `WalletTenantConfigR2dbcAdapter` UPSERT, including `updated_by`), FK to `public.tenant_registry`, `wallet_mode`/`key_manager` CHECK constraints (kebab-case values: `browser`/`server`, `db-tde`/`hybrid`/`hsm`/`qtsp`; FR-20 browser ⇒ no key_manager), and an index on `host`. Added `db/tenant/V3__Wallet_config_audit.sql` creating `<tenant>_business_wallet.wallet_config_audit` (column set aligned with the `TenantConfigAuditR2dbcAdapter` INSERT) plus a `BEFORE UPDATE OR DELETE` trigger that makes the table append-only. These tables were previously declared in `eudistack-platform-iac/db/migration/`, which has no Flyway runner — they belong with the EBW (which owns and migrates the `public` schema via `TenantSchemaFlywayMigrator`), as `tenant_wallet_config` is a sibling of `tenant_registry` (feature-design §4.2).

### Security

- **EUDISTACK-412 (SEC-B1) — strict schema-name validation on the admin write / audit path.** `schemaName` (which routes audit writes to `<schemaName>_business_wallet.wallet_config_audit` and is the `tenant_wallet_config` key) is validated as a strict PostgreSQL identifier (`^[a-z][a-z0-9_]{0,62}$`) at the persistence-adapter boundary and on the `PUT .../{schemaName}` path variable — an invalid value is rejected with HTTP 400 (RFC 9457) before any SQL runs, closing a SQL-identifier-injection / cross-tenant audit-write vector.

- **EUDISTACK-412 (SEC-B2) — admin endpoint authenticated via a static `X-Admin-Api-Key` header.** `AdminApiKeyAuthenticationWebFilter` requires the `X-Admin-Api-Key` request header on `/admin/**`, comparing it in constant time (`MessageDigest.isEqual`) against `ebw.admin.api-key` (env `ADMIN_API_KEY`). When the key is unconfigured the filter **fails closed** — every `/admin/**` request is rejected with 401 (and a WARN is logged at startup). A missing/wrong header → 401 with an RFC 9457 `application/problem+json` body (`type=urn:eudistack:error:admin-auth-required`). On `/admin/**` the API key is the only accepted credential; a bearer JWT alone → 401. On a valid key the filter establishes a security context bearing `SCOPE_tenant.config.write`, so the existing `SecurityConfig` URL rule and the `@PreAuthorize` on the handlers pass as defence in depth. This supersedes the earlier (never-functional) assumption that the endpoint would be gated by a `tenant.config.write` JWT scope — no EBW token mints that scope. STG/PROD supply `ADMIN_API_KEY` via an SSM parameter; the local dev value is in `eudistack-platform-dev/compose.yaml`.
