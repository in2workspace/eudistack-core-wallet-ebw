 ...
 *
 * <p>EUDISTACK-412 cases: 200 with the 4-field projection (no {@code key_manager}/{@code activation_status}) +
 ...
 *
 * <p>EUDISTACK-413 cases: 200 for a {@code server}+{@code db-tde} tenant with explicit absence
 * assertion for {@code key_manager}/{@code activation_status} (AC-1b, AC-1d, AD-1bis); uppercase
 * Host and Host with port → 200 (AC-1e); unregistered server host → 404 opaque, same #calls (AC-5a,
 * AC-5b).
 *
...
class WalletTenantConfigDiscoveryControllerIT {
    ...
    // EUDISTACK-413: server+db-tde tenant host (mimics DOME production host)
    ...
    // ------------------------------------------------------------------
    // EUDISTACK-413 — server+db-tde: 200 with 4-field projection + no key_manager (AC-1b, AC-1d)
    // ------------------------------------------------------------------

    /**
     * EUDISTACK-413 AC-1b / AC-1d: a {@code server}+{@code db-tde} tenant returns HTTP 200 with
     * the 4-field projection {@code {wallet_mode:"server", natural_persons_only:false,
     * supported_credentials:[], version:N}} and caching headers. The body must NOT contain
     * {@code key_manager} or {@code activation_status} (AD-1bis). Although {@link DiscoveryResponseDto}
     * structurally cannot carry those fields, this test makes the intent explicit and guards against
     * future accidental field additions.
     */
    @Test
    void getServerDbTdeTenant_returns200WithServerModeInFourFieldProjectionAndNoCachingKeyManager() {
        // Given
        TenantWalletConfigDescriptor descriptor = TenantWalletConfigDescriptor.of(
                "dome", SERVER_HOST, WalletMode.SERVER,
                Optional.of(KeyManager.DB_TDE), false, Collections.emptyList(), 1L);
        when(readService.retrieveDescriptor(SERVER_HOST)).thenReturn(Mono.just(descriptor));

        // When
        Mono<ResponseEntity<Object>> result = controller.discover(SERVER_HOST);
        ...
        StepVerifier.create(result)
                .assertNext(response -> {
                    ...
                    HttpHeaders headers = response.getHeaders();
                    assertThat(headers.getCacheControl()).contains("max-age=60").contains("public");
                    assertThat(headers.getETag()).isEqualTo("\"1\"");
                    assertThat(headers.getVary()).containsExactly(HttpHeaders.HOST);

                    // 4-field projection: wallet_mode=server, no key_manager, no activation_status (AD-1bis).
                    assertThat(response.getBody()).isInstanceOf(DiscoveryResponseDto.class);
                    DiscoveryResponseDto dto = (DiscoveryResponseDto) response.getBody();
                    ...
                    // Explicit absence check: DiscoveryResponseDto must not expose key_manager (AD-1bis).
        ...
        verify(readService, times(1)).retrieveDescriptor(SERVER_HOST);
    }

    // ------------------------------------------------------------------
    // EUDISTACK-413 AC-1e — server tenant: uppercase Host → normalised → 200
    // ------------------------------------------------------------------

    /**
     * EUDISTACK-413 AC-1e: an uppercase {@code Host} for a {@code server} tenant is normalised to
     * lowercase before the port call — same behaviour as for {@code browser} (inherited from
     * EUDISTACK-412; explicitly covered here for the {@code server} case).
     */
    @Test
    void getServerTenant_uppercaseHost_isNormalisedToLowercaseAndReturns200() {
        // Given — the service is stubbed for the *lowercase* host only
        TenantWalletConfigDescriptor descriptor = TenantWalletConfigDescriptor.of(
                "dome", SERVER_HOST, WalletMode.SERVER,
                Optional.of(KeyManager.DB_TDE), false, Collections.emptyList(), 1L);
        ...
        // When — caller sends the uppercase variant
        Mono<ResponseEntity<Object>> result = controller.discover(SERVER_HOST.toUpperCase());

        // Then — the controller lowercased it before the lookup → 200
        StepVerifier.create(result)
                .assertNext(response -> {
                    assertThat(response.getStatusCode()).isEqualTo(HttpStatus.OK);
                    DiscoveryResponseDto dto = (DiscoveryResponseDto) response.getBody();
    ...
    /**
     * EUDISTACK-413 AC-1e: a {@code Host} header with port suffix for a {@code server} tenant is
     * normalised (port stripped) before the port call.
     */
    @Test
    void getServerTenant_hostWithPort_isPortStrippedAndReturns200() {
        // Given — the service is stubbed for the bare lowercase host
        TenantWalletConfigDescriptor descriptor = TenantWalletConfigDescriptor.of(
                "dome", SERVER_HOST, WalletMode.SERVER,
                Optional.of(KeyManager.DB_TDE), false, Collections.emptyList(), 1L);
        when(readService.retrieveDescriptor(SERVER_HOST)).thenReturn(Mono.just(descriptor));

        // When — caller sends host with :443 port
        Mono<ResponseEntity<Object>> result = controller.discover(SERVER_HOST + ":443");

        // Then — the controller stripped the port → stub matched → 200
        StepVerifier.create(result)
                .assertNext(response -> {
                    assertThat(response.getStatusCode()).isEqualTo(HttpStatus.OK);
                    DiscoveryResponseDto dto = (DiscoveryResponseDto) response.getBody();
    ...
    // ------------------------------------------------------------------
    // EUDISTACK-413 AC-5a / AC-5b — unregistered server host → opaque 404, same #calls
    // ------------------------------------------------------------------

    /**
     * EUDISTACK-413 AC-5a / AC-5b: a host belonging to an unseeded {@code server} tenant returns
     * an opaque RFC 9457 404 and goes through exactly the same number of read-service calls as
     * the 200 (anti-timing-oracle). The 404 is not cached ({@code Cache-Control} absent).
     */
    @Test
    void getUnregisteredServerHost_returns404OpaqueRfc9457_throughSameNumberOfReadCallsAsThe200() {
        // Given
        String unseededServerHost = "unknown-server.eudistack.example.com";
        when(readService.retrieveDescriptor(unseededServerHost)).thenReturn(Mono.empty());

        // When
        Mono<ResponseEntity<Object>> result = controller.discover(unseededServerHost);
        ...
        StepVerifier.create(result)
                .assertNext(response -> {
                    ...
                    assertThat(problem.getTitle()).isEqualTo("Not Found");
                    assertThat(problem.getStatus()).isEqualTo(404);
                    // No cause discriminator — body must not reveal whether the host exists.
                    assertThat(problem.getDetail()).isNull();
                })
                .verifyComplete();

        // AC-5b: the 404 path makes exactly one read-service call — same as the 200 path.
        verify(readService, times(1)).retrieveDescriptor(unseededServerHost);
    }
}
